...
\missingfigure{Linear Regression from Hedley}
...
The following algorithm is based in the method developed by \cite{Gordon:1994} for retrieval of water-leaving radiance and aerosol optical thickness over the oceans with SeaWiFS. This method is still applied to the basic SeaWiFS and MODIS atmospheric correction algorithms for Case 1 water.

The first step in this algorithm is performing a Rayleigh scattering correction, which means to subtract the reflectance due to Rayleigh scatter $\rho_r$ from the total TOA reflectance $\rho_t$ in \autoref{eq:rhodeglint}. This Rayleigh scattering component $\rho_r$ can be calculated directly from MODTRAN for the particular illumination and viewing geometry of the sun and the sensor in multiple-scatter mode but without aerosol. After calculating this component, \autoref{eq:rhodeglint} becomes 
...
\end{equation}
where $\rho_c(\lambda)$ is the Rayleigh-corrected reflectance. If we define the total multiple-scattering aerosol reflectance $\rho_{am}$ as
...
There are two different approach to determine $\rho_{am}(\lambda)$ at this point. The first one is to use single-scattering approximation. The second one is to determine the multiple-scattering term $\rho_{am}(\lambda)$ based in the single scattering term.

We need to calculate this total multiple-scattering aerosol reflectance $\rho_{am}$ in order to obtain the desired water-leaving reflectance $\rho_w$. For Case 1 waters, the term $\rho_{w}$ is assumed to be zero for NIR bands. If we take the SeaWiFS case for band 7 ($\lambda_7=765nm$) and band 8 ($\lambda_8=865nm$), then
...
\begin{equation}\label{eq:seawifsam8}
    \rho_{am}^{(8)}=\rho_{c}^{(8)},
\end{equation}
where $\rho_{x}^{(i)}$ denotes a reflectance at band $i$ with wavelength $\lambda_i$. \autoref{eq:seawifsam7} and \autoref{eq:seawifsam8} mean that the Rayleigh-corrected reflectance is equal to only the aerosol and aerosol-Rayleigh scattering in the atmosphere for band 7 and band 8 in SeaWiFS. Now, we need to find a way to use this result to calculate the atmospheric reflectance for all bands.

\cite{Gordon:1994} define the atmospheric-correction parameter named single scattering epsilon (SSE)(\cite{IOCCG:2010}) $\varepsilon_{s(I)}^{(i,j)}$ (with the notation used by \cite{Ruddick:2000bs}) as
...
\cite{Wang:1991} has shown that a near-linear relation exists between $\rho_a(\lambda)+\rho_{ra}(\lambda)$ for the multiple-scattering model and $\rho_{as}(\lambda)$ for the single-scattering model.
