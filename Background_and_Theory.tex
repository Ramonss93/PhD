...
\todo{Explain the original ELM} 
The ELM-based algorithm uses HydroLight for the estimation of the dark pixel field spectra and the Landsat reflectance product \todo{Explain CDR reflectance product.} (Landsat Surface Reflectance CDR;\cite{LandsatCDR}) for the estimation of the bright pixel spectra data.
...
\begin{figure}
  ...
  \begin{tikzpicture}[node distance=0.75cm, auto]
          ...
          \node[basenode, right=of SWIR] (Ratio) {Ratio NIR Band/ Red Band};
          \node[basenode, below=of Ratio] (TS2) {Mask by Threshold};
          \node[basenode] (AND) [below right= 1cm of TS1] {AND}
  ...
  \end{tikzpicture}
\caption{PIF flow chart \label{fig:PIFflowchart} } 
\end{figure}
