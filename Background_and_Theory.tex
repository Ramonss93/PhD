...
\autoref{fig:radiance} shows the geometry used to define the radiometric quantities. We need to define a fundamental radiometric quantity that specifies completely positional ($x,y,z$), temporal ($t$), directional ($\theta,\phi$) and spectral ($\lambda$) structure of the light field. This is accomplished by the spectral radiance $L$. The {\it spectral radiance} $L$ \index{\it spectral radiance, $L$} can be defined as (\cite{Mobley:2001})
...
Even though the spectral radiance describes completely the light field, it is not commonly used for instrumentation difficulties or for all that information is not needed. Irradiance is a radiometric quantity that is easier to measure and ofter more useful. Consider a light detector that is equally sensitive to photons in any direction within a hemisphere of directions. If that detector is placed at depth $z$, oriented facing upward and therefore collection photons coming from the downward direction, the this detector is measuring the spectral downwelling scalar irradiance at depth $z$, $E_{od}(x,\lambda)$. This spectral downwelling scalar irradiance is can be defined as
...
If the same instrument is place facing downward, the it will be measuring the spectral upwelling scalar irradiance $ E_{ou}(z,\lambda)$, defined as
\begin{equation}
